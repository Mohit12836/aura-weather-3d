...
export const HourlyForecast: React.FC<HourlyForecastProps> = ({ hourly, unit }) => {
  ...
  return (
    <div className="w-full rounded-3xl border border-white/10 bg-slate-900/60 backdrop-blur-xl p-5 md:p-6 shadow-xl">
      ...
      <div className="flex items-center gap-3 overflow-x-auto pb-3 pt-1 no-scrollbar select-none">
        {hourly.map((item, idx) => {
          ...
          return (
            <motion.div
              ...
              className={`flex-shrink-0 flex flex-col items-center justify-between p-3.5 rounded-2xl border min-w-[85px] transition-colors ${
                isNow
                  ? 'bg-gradient-to-b from-amber-500/20 to-orange-500/10 border-amber-500/40 shadow-lg shadow-amber-500/10'
                  : 'bg-white/5 border-white/10 hover:bg-white/10'
              }`}
            >
              <span className={`text-xs font-semibold ${isNow ? 'text-amber-300 font-bold' : 'text-slate-400'}`}>
                {item.formattedHour}
              </span>
              ...
              <div className="flex items-center gap-1 text-[11px] mt-2 font-medium">
                <Droplets className={`w-3 h-3 ${item.precipitationProbability > 20 ? 'text-sky-400' : 'text-slate-500'}`} />
