  ...
  return (
    <div className="w-full rounded-3xl border border-white/10 bg-slate-900/60 backdrop-blur-xl p-5 md:p-6 shadow-xl">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xs text-slate-400">Week Outlook</span>
      </div>
        ...
        {daily.map((item, idx) => {
          ...
          return (
            <motion.div
              ...
              className="py-3 flex items-center justify-between gap-2 md:gap-4 transition-colors hover:bg-white/[0.02] rounded-xl px-1"
            >
              {/* Day Name */}
              <div className="w-20 md:w-24">
                <span className={`text-xs md:text-sm font-semibold ${isToday ? 'text-amber-400 font-bold' : 'text-slate-200'}`}>
                  {item.dayName}
                </span>
              ...
              <div className="flex items-center gap-2 w-20 md:w-28">
                ...
                {item.precipitationProbability > 15 && (
                  <div className="flex items-center gap-0.5 text-[11px] text-sky-400 font-semibold">
