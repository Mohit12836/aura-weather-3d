...
export const HeroWeatherCard: React.FC<HeroWeatherCardProps> = ({ weather, unit }) => {
  ...
  const renderConditionIcon = () => {
    ...
    switch (condition.icon) {
      case 'Sun': return <Sun {...props} className="w-6 h-6 text-amber-400 animate-spin-slow" />;
      ...
      case 'CloudRain': return <CloudRain {...props} className="w-6 h-6 text-sky-400" />;
      case 'CloudSnow': return <CloudSnow {...props} className="w-6 h-6 text-cyan-200" />;
      case 'CloudLightning': return <CloudLightning {...props} className="w-6 h-6 text-purple-400 animate-pulse" />;
      case 'CloudFog': return <CloudFog {...props} className="w-6 h-6 text-slate-300" />;
      default: return <Cloud {...props} className="w-6 h-6 text-slate-300" />;
    }
  };
  ...
  return (
    <SpotlightCard
      ...
      className="w-full relative overflow-hidden border border-white/15 bg-gradient-to-br from-slate-900/90 via-slate-900/60 to-slate-950/80 p-6 md:p-8 rounded-3xl"
    >
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-center">
        ...
        <div className="lg:col-span-7 flex flex-col justify-between z-10">
          ...
          <div className="flex flex-wrap items-center gap-2 mb-4">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/10 backdrop-blur-md border border-white/10 text-xs font-semibold text-slate-200">
              <Navigation className="w-3.5 h-3.5 text-amber-400 fill-amber-400/20" />
              <span>{location.name}</span>
              <span className="text-slate-400">({location.country})</span>
            </div>

            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-500/10 border border-amber-500/25 text-xs font-bold text-amber-300">
          ...
          <div className="flex items-baseline gap-2 my-2">
            ...
            <span className="text-3xl md:text-5xl font-light text-amber-400 select-none">
              {unit}
            </span>
          </div>
          ...
          {/* High / Low / Feels Like Pills */}
          <div className="flex flex-wrap items-center gap-3 mt-6 pt-4 border-t border-white/10">
            <div className="flex items-center gap-1.5 text-xs md:text-sm font-medium text-slate-300">
            ...
            <div className="flex items-center gap-1.5 text-xs md:text-sm font-medium text-emerald-400">
              <ArrowUp className="w-4 h-4" />
              <span>High: <strong><AnimatedNumber value={maxTemp} suffix={unit} /></strong></span>
            </div>

            <div className="flex items-center gap-1.5 text-xs md:text-sm font-medium text-sky-400">
            ...
            <div className="flex items-center gap-1 text-xs text-amber-300/90 font-medium ml-auto">
        ...
        <div className="lg:col-span-5 relative w-full h-[260px] md:h-[320px] rounded-2xl overflow-hidden bg-radial from-white/5 to-transparent flex items-center justify-center">
          <WeatherScene3D 
          ...
          <div className="absolute bottom-2 right-3 pointer-events-none px-2.5 py-1 rounded-full bg-black/40 backdrop-blur-md border border-white/10 text-[10px] text-slate-400 font-mono flex items-center gap-1.5">
