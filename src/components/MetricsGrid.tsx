...
export const MetricsGrid: React.FC<MetricsGridProps> = ({ weather, unit }) => {
  ...
  return (
    <div className="grid-auto-fit">
      ...
      <SpotlightCard glowColor="rgba(56, 189, 248, 0.2)">
        <div className="flex items-center justify-between text-slate-400 text-xs font-semibold uppercase tracking-wider mb-3">
          <span className="flex items-center gap-1.5">
            <Wind className="w-4 h-4 text-sky-400" /> Wind Flow
          </span>
          <span className="font-mono text-slate-300">{current.windDirection}°</span>
        </div>

        <div className="flex items-center justify-between gap-4">
          <div>
            <div className="fluid-h2 font-black text-white">
              <AnimatedNumber value={windSpeedValue} suffix={` ${windUnit}`} />
            </div>
            <p className="text-xs text-slate-400 mt-1">
              Gusts & atmospheric currents
            </p>
          </div>

          {/* Rotating Direction Compass */}
          <div className="relative w-12 h-12 rounded-full border border-white/20 bg-white/5 flex items-center justify-center shadow-inner">
            <Compass 
              className="w-7 h-7 text-sky-400 transition-transform duration-700" 
      ...
      {/* 2. Humidity */}
      <SpotlightCard glowColor="rgba(14, 165, 233, 0.2)">
        <div className="flex items-center justify-between text-slate-400 text-xs font-semibold uppercase tracking-wider mb-3">
          <span className="flex items-center gap-1.5">
        ...
        <p className="text-xs text-slate-400 mt-2">
          Moisture saturation index
        </p>
      </SpotlightCard>

      {/* 3. UV Index */}
      <SpotlightCard glowColor="rgba(245, 158, 11, 0.2)">
        <div className="flex items-center justify-between text-slate-400 text-xs font-semibold uppercase tracking-wider mb-3">
          <span className="flex items-center gap-1.5">
            <SunMedium className="w-4 h-4 text-amber-400" /> UV Index
          </span>
          <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold border ${uvInfo.bg} ${uvInfo.color}`}>
            {uvInfo.label}
          </span>
        </div>
        ...
        <p className="text-xs text-slate-400 mt-2">
          Solar radiation exposure level
        </p>
      </SpotlightCard>

      {/* 4. Air Quality Index (AQI) */}
      <SpotlightCard glowColor="rgba(34, 197, 94, 0.2)">
        <div className="flex items-center justify-between text-slate-400 text-xs font-semibold uppercase tracking-wider mb-3">
          <span className="flex items-center gap-1.5">
        ...
        <div className="flex items-center justify-between text-xs text-slate-400 mt-3 pt-2 border-t border-white/10">
          <span>PM2.5: <strong className="text-slate-200">{airQuality?.pm25 ?? 12} µg/m³</strong></span>
          <span>PM10: <strong className="text-slate-200">{airQuality?.pm10 ?? 24} µg/m³</strong></span>
        </div>
      </SpotlightCard>

      {/* 5. Atmospheric Pressure */}
      <SpotlightCard glowColor="rgba(168, 85, 247, 0.2)">
        <div className="flex items-center justify-between text-slate-400 text-xs font-semibold uppercase tracking-wider mb-3">
          <span className="flex items-center gap-1.5">
        ...
        <p className="text-xs text-slate-400 mt-3">
          {current.surfacePressure > 1013 ? 'High pressure system (Calm)' : 'Low pressure system (Active)'}
        </p>
      </SpotlightCard>

      {/* 6. Visibility */}
      <SpotlightCard glowColor="rgba(59, 130, 246, 0.2)">
        <div className="flex items-center justify-between text-slate-400 text-xs font-semibold uppercase tracking-wider mb-3">
          <span className="flex items-center gap-1.5">
            <Eye className="w-4 h-4 text-blue-400" /> Visibility
          </span>
          <span className="text-xs text-slate-300">
            {current.visibility >= 10 ? 'Optimal' : (current.visibility >= 5 ? 'Fair' : 'Hazy')}
          </span>
        </div>
